...
def from_directory(tokenize, path,in_network_SCOTUS_timespan_opinions_list):
        """
        ...
        network_size=len(in_network_SCOTUS_timespan_opinions_list)
        for name in in_network_SCOTUS_timespan_opinions_list:
                loader.add_document(tokenize(open(path+"/"+name+".json").read()));
                print "\it is the name of the opinion", name
                DTM_title_list.append(name)
                n=n+1
                print "n:  ",n, "from ",network_size," opinions in the network"
        print "final n: ", n
        print loader.done()
        return loader.done(),DTM_title_list;
